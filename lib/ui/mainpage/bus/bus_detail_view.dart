  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      onPopInvokedWithResult: (didPop, result) async {
        mainMapViewModel.removeMarkerHistory();

        // 맵을 새로 고침하여 정상적인 마커 상태 유지
        mainMapViewModel.onMapCreated(context, mainMapViewModel.mapController!);
      },
      child: ChangeNotifierProvider<BusDetailViewModel>(
        create: (_) =>
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (busDetailViewModel.errorMessage != null) {
                ScaffoldMessenger.of(context).showSnackBar(
                  CustomSnackbar(
                      content: Text(busDetailViewModel.errorMessage!)),
                );
                ...
                actions: [
                  IconButton(
                    onPressed: () {
                      // 버스 새로고침
                      busDetailViewModel
                          .refreshBusInfo(busStopInfoModel.lineid!);
                    },
